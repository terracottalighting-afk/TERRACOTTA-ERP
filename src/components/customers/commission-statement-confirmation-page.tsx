...
export async function CommissionStatementConfirmationPage({ agencyId, confirmAction, error, invoiceIds }: { agencyId?: string; confirmAction: FormAction; error?: string; invoiceIds?: string }) {
  ...
  const [{ data: agency, error: agencyError }, { data: snapshots, error: snapshotsError }, { data: invoices, error: invoicesError }] = await Promise.all([
  ...
  if (agencyError || snapshotsError || invoicesError) throw new Error(agencyError?.message ?? snapshotsError?.message ?? invoicesError?.message ?? "Unable to load commission statement details.");
