...
async function FinancialDashboardTabs({
  financialFilters,
  financialTab,
  packingLists,
}: {
  ...
  financialTab?: string;
  packingLists: Awaited<ReturnType<typeof getInvoiceQueuePackingLists>>;
}) {
  const supabase = createSupabaseAdminClient();
                  ...
                  {allPayments.map((payment) => {
                    ...
                    return (
                      <tr key={payment.id}>
                        ...
                        <td>{money(Number(payment.amount_received))}</td>
                        <td>
                          <StatusBadge
                            tone={
...
async function LocationInfoPage({
  customerId,
  locationId,
}: {
  customerId?: string;
  ...
  const [customer, dashboard] = await Promise.all([
    ...
    getLocationDashboard(customerId, locationId),
  ]);
  const {
    contacts,
    displays,
