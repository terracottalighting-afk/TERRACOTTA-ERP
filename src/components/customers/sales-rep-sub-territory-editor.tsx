...
export async function SalesRepSubTerritoryEditor({ error, salesRepId, saveAction }: { error?: string; salesRepId?: string; saveAction: FormAction }) {
  ...
  const [{ data: agencyAssignments, error: agencyAssignmentsError }, { data: repAssignments, error: repAssignmentsError }] = await Promise.all([
    ...
    supabase.from("sales_rep_territory_assignment").select("territory_id").eq("sales_rep_id", salesRep.id).eq("status", "active").is("end_date", null),
  ]);
  if (agencyAssignmentsError || repAssignmentsError) throw new Error(agencyAssignmentsError?.message ?? repAssignmentsError?.message);
  ...
  return <section className="dashboard-panel"><section className="form-header"><div><span className="eyebrow">Sales Coverage</span><h2>Add Sub-Territory</h2><p>Assign base territories from the parent agency to {salesRep.name}.</p></div><Link className="secondary-action secondary-action--light" href={returnUrl}>Cancel</Link></section>{error ? <div className="form-alert">{decodeURIComponent(error)}</div> : null}<form action={saveAction} className="customer-form"><input name="sales_rep_id" type="hidden" value={salesRep.id} /><fieldset><legend>Available Agency Territories</legend><p className="fieldset-note">Only territories assigned to this sales rep’s agency are available.</p><div className="territory-assignment-grid">{eligibleTerritories.map((territory) => <label className="checkbox-label" key={territory.id}><input name="territory_ids" type="checkbox" value={territory.id} /><span><strong>{territory.name}</strong><small>{territory.territory_code}{territory.description ? ` - ${territory.description}` : ""}</small></span></label>)}</div>{!eligibleTerritories.length ? <p className="fieldset-note">There are no unassigned agency territories available for this sales rep.</p> : null}</fieldset><div className="form-actions"><button className="primary-action" disabled={!eligibleTerritories.length} type="submit">Add Selected Sub-Territories</button><Link className="secondary-action secondary-action--light" href={returnUrl}>Cancel</Link></div></form></section>;
}
