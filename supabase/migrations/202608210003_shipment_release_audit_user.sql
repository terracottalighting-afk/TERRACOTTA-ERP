-- Until application authentication is wired into the server actions, shipment
-- posting must still satisfy the audit fields required by freight_shipment.
-- Prefer the caller identity, then a shipment creator, then an active internal
-- user as the controlled Phase 1 fallback.
...
begin
  ...
  if effective_user_id is null then
    select id into effective_user_id
    from user_account
    where is_active = true and user_type = 'internal'
    order by created_at, id
    limit 1;
  end if;
  if effective_user_id is null then
    raise exception 'An active internal ERP user is required to post a shipment.';
  end if;
  ...
  loop
    ...
    set status = 'shipped',
        ...
        packed_by_user_id = coalesce(packed_by_user_id, effective_user_id),
        updated_at = now()
