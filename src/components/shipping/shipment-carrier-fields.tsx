...
export function ShipmentCarrierFields({ carriers, currentCarrier = "", currentCarrierAccountNumber = "", currentShippingType = "", customerCarriers = [] }: { carriers: Carrier[]; currentCarrier?: string; currentCarrierAccountNumber?: string; currentShippingType?: string; customerCarriers?: CustomerCarrier[] }) {
  ...
  const [useCustomerCarriers, setUseCustomerCarriers] = useState(false);
  ...
  return <>
    ...
    {customerCarriers.length ? <label className="inline-checkbox full-width-field">Use Customer Carriers<input checked={useCustomerCarriers} name="use_customer_carriers" onChange={(event) => setUseCustomerCarriers(event.target.checked)} type="checkbox" /></label> : null}
