-- Apply an optional posted credit memo and the remaining customer payment to one invoice atomically.
-- Existing credit memo and AR triggers refresh balances and statuses after each application is posted.
create or replace function public.record_invoice_settlement_with_credit_memo(
...
begin
  ...
  settlement_total := p_customer_payment_amount + p_credit_memo_amount;

  if p_customer_payment_amount < 0 or p_credit_memo_amount < 0 then
    raise exception 'Payment and credit memo amounts cannot be negative';
  end if;

  if settlement_total <= 0 then
    raise exception 'Apply a credit memo amount, record a customer payment, or both';
  end if;
  ...
  if p_credit_memo_id is not null then
    select * into credit_memo_row
    from credit_memo
    where id = p_credit_memo_id
    for update;

    if not found then
      raise exception 'The selected credit memo does not exist';
    end if;
    ...
    where credit_memo_id = credit_memo_row.id
  ...
  if p_credit_memo_amount > 0 then
    insert into credit_memo_application (
      ...
      customer_invoice_id,
      customer_payment_id,
      customer_payment_application_id,
      amount_applied,
...
grant execute on function public.record_invoice_settlement_with_credit_memo(uuid, date, customer_payment_method, numeric, uuid, numeric, text, text, uuid) to service_role;
