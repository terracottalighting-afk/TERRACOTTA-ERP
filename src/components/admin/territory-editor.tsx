...
export async function TerritoryEditor({ createAction, deactivateAction, error, notice, saveAction, territoryId }: { createAction: FormAction; deactivateAction: FormAction; error?: string; notice?: string; saveAction: FormAction; territoryId?: string }) {
  ...
  return <section className="dashboard-panel"><section className="account-header"><div><span className="eyebrow">Territory Settings</span><h2>{editing ? `Edit ${territory?.name}` : "Add Territory"}</h2><Link className="text-action" href="/?module=admin&admin_tab=territory">Back to Territory Settings</Link></div></section>{error ? <p className="form-error">{decodeURIComponent(error)}</p> : null}{notice ? <p className="form-notice">{decodeURIComponent(notice)}</p> : null}<form action={editing ? saveAction : createAction} className="form-stack">{editing ? <input name="territory_id" type="hidden" value={territory?.id ?? ""} /> : null}<fieldset><legend>Territory Profile</legend><div className="form-grid"><label>Territory Code<input defaultValue={territory?.territory_code ?? ""} name="territory_code" required /></label><label>Territory Name<input defaultValue={territory?.name ?? ""} name="name" required /></label><label className="full-width-field">Description<textarea defaultValue={territory?.description ?? ""} name="description" /></label>{editing ? <label>Status<select defaultValue={territory?.status ?? "active"} name="status"><option value="active">Active</option><option value="inactive">Inactive</option></select></label> : null}</div></fieldset><fieldset><legend>Coverage</legend><TerritoryCoverageFields countyRules={countyRules} excludedPostalCodes={excludedPostalCodes} includedPostalCodes={includedPostalCodes} resolvedZipCount={resolvedZipCount} selectedFullStates={selectedStates} /></fieldset><div className="form-actions"><button className="primary-action" type="submit">{editing ? "Save Territory Changes" : "Create Territory"}</button>{editing && territory?.status === "active" ? <button className="danger-action" formAction={deactivateAction} type="submit">Deactivate Territory</button> : null}<Link className="secondary-action" href="/?module=admin&admin_tab=territory">Cancel</Link></div></form></section>;
}
